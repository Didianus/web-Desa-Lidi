...
export function LayananSuratPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true)
      setForm({ nama: '', nik: '', alamat: '', jenisSurat: '', keterangan: '' })
      toast({ title: 'Berhasil', description: 'Pengajuan surat berhasil dikirim' })
    } catch {
      toast({ title: 'Error', description: 'Gagal mengirim pengajuan surat', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-emerald-200 mt-2">Ajukan surat secara online dengan mudah</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-10">
          {jenisSurat.map((js) => (
            <Card key={js.value} className="border-0 shadow-md hover:shadow-lg transition-shadow cursor-pointer"
              onClick={() => setForm(f => ({ ...f, jenisSurat: js.value }))}>
              <CardContent className="p-5 text-center">
                <span className="text-3xl">{js.icon}</span>
                <p className="font-medium text-gray-900 mt-2 text-sm">{js.label}</p>
              </CardContent>
            </Card>
          ))}
        </div>
                ...
                {success ? (
                  ...
                  <>
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h2 className="text-xl font-bold text-gray-900">Form Pengajuan Surat</h2>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-5">
                      <div>
                        <Label htmlFor="nama">Nama Lengkap *</Label>
                        <Input id="nama" value={form.nama} onChange={e => setForm(f => ({ ...f, nama: e.target.value }))} placeholder="Masukkan nama lengkap" className="mt-1" />
                      </div>
                      <div>
                        <Label htmlFor="nik">NIK *</Label>
                        <Input id="nik" value={form.nik} onChange={e => setForm(f => ({ ...f, nik: e.target.value }))} placeholder="Masukkan NIK (16 digit)" maxLength={16} className="mt-1" />
                      </div>
                      <div>
                        <Label htmlFor="alamat">Alamat *</Label>
                        <Textarea id="alamat" value={form.alamat} onChange={e => setForm(f => ({ ...f, alamat: e.target.value }))} placeholder="Masukkan alamat lengkap" className="mt-1" />
                      </div>
                      <div>
                        <Label>Jenis Surat *</Label>
                        <Select value={form.jenisSurat} onValueChange={v => setForm(f => ({ ...f, jenisSurat: v }))}>
                      ...
                      <div>
                        ...
                        <Textarea id="keterangan" value={form.keterangan} onChange={e => setForm(f => ({ ...f, keterangan: e.target.value }))} placeholder="Keterangan tambahan (opsional)" className="mt-1" />
                      </div>
          ...
          <div className="md:col-span-2 space-y-4">
            <Card className="border-0 shadow-md">
              <CardContent className="p-6">
                <h3 className="font-bold text-gray-900 mb-4">Informasi Layanan</h3>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">1-3 hari kerja setelah pengajuan</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <FileText className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">Pengambilan</p>
                      <p className="text-xs text-gray-500">Di kantor desa pada jam pelayanan</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-0 shadow-md bg-emerald-50">
              <CardContent className="p-6">
                <h3 className="font-bold text-emerald-800 mb-2">Syarat Pengajuan</h3>
                <ul className="text-sm text-emerald-700 space-y-2">
                  <li>✅ Warga Desa Sukamaju</li>
