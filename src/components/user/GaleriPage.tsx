    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-emerald-200 mt-2">Dokumentasi kegiatan dan kehidupan Desa Sukamaju</p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map((item, idx) => (
              <Card key={item.id} className="border-0 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group cursor-pointer">
                <div className={`aspect-square bg-gradient-to-br ${placeholderColors[idx % placeholderColors.length]} relative overflow-hidden`}>
                  {item.image ? (
                ...
                <CardContent className="p-3">
                  ...
                  <Badge className={`${categoryColors[item.category] || 'bg-gray-100 text-gray-700'} text-xs mt-1`}>
                    {item.category}
                  </Badge>
                </CardContent>
              </Card>
