...
import { Plus, Trash2, Pencil, Image as ImageIcon, Loader2, ChevronLeft, ChevronRight, Download, FileJson, FileSpreadsheet, Search, X } from 'lucide-react'
...
export function GaleriManager() {
  ...
  const [form, setForm] = useState({ title: '', image: '', category: 'umum', album: '' })
  ...
  const allAlbums = Array.from(new Set(list.map(item => item.album).filter(Boolean)))
  ...
  const fetchData = () => {
    ...
    fetch(`/api/galeri?${params}`)
      .then(r => r.json())
      .then(d => { setList(d.galeri || []); setTotal(d.total || 0); setLoading(false) })
      .catch(() => { setLoading(false) })
  }

  useEffect(() => { fetchData() }, [page, filterCategory, filterAlbum])
  ...
  const handleSave = async () => {
    ...
    if (allImages.length === 0) {
      toast({ title: 'Error', description: 'Minimal satu gambar harus diisi', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      toast({ title: 'Berhasil', description: `Foto berhasil ${editing ? 'diupdate' : 'ditambahkan'}` })
      setDialogOpen(false); setEditing(null); setForm({ title: '', image: '', category: 'umum', album: '' }); setImageUrls([])
      setLoading(true)
      fetchData()
    } catch { toast({ title: 'Error', description: `Gagal ${editing ? 'mengupdate' : 'menambahkan'} foto`, variant: 'destructive' }) }
  ...
  const handleExport = async (format: string) => {
    try {
      ...
      if (format === 'csv') {
        ...
        a.href = url; a.download = `galeri_${new Date().toISOString().split('T')[0]}.csv`; a.click()
        ...
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url; a.download = `galeri_${new Date().toISOString().split('T')[0]}.json`; a.click()
        URL.revokeObjectURL(url)
      }
  ...
  const openEdit = (item: GaleriItem) => {
    setEditing(item)
    setForm({ title: item.title, image: item.image || '', category: item.category, album: item.album || '' })
    try {
  ...
  const categoryBadge: Record<string, string> = {
    umum: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400', kegiatan: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
    pembangunan: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', budaya: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
  }

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">Kelola Galeri</h1><p className="text-gray-500 dark:text-gray-400 text-sm">Kelola foto galeri desa</p></div>
        <div className="flex items-center gap-2">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto dark:bg-gray-900">
              <DialogHeader><DialogTitle className="dark:text-white">{editing ? 'Edit Foto' : 'Tambah Foto'}</DialogTitle></DialogHeader>
              <div className="space-y-4 pt-4">
                ...
                <div>
                  <label className="text-sm font-medium dark:text-gray-300">Kategori</label>
                ...
                <div>
                  <label className="text-sm font-medium dark:text-gray-300">Album</label>
                  <div className="flex gap-2 mt-1">
                    <Select value={form.album && allAlbums.includes(form.album) ? form.album : '__new__'} onValueChange={v => {
                      if (v !== '__new__') setForm(f => ({ ...f, album: v }))
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {loading ? (
          [1,2,3,4].map(i => <div key={i} className="aspect-square bg-gray-200 dark:bg-gray-700 rounded-xl animate-pulse" />)
        ) : list.length === 0 ? (
          <div className="col-span-full text-center py-20 text-gray-400">Belum ada foto</div>
        ) : list.map(item => (
          <Card key={item.id} className="border-0 shadow-md overflow-hidden group dark:bg-gray-900 dark:border dark:border-gray-800">
            <div className="aspect-square bg-gradient-to-br from-emerald-400 to-emerald-600 relative">
              ...
              {item.album && (
                <div className="absolute top-2 left-2">
                  <Badge className="bg-black/50 text-white text-[10px]">{item.album}</Badge>
                </div>
              )}
            </div>
