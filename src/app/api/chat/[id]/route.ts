...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await db.chatRoom.findUnique({ where: { id } })
    if (!existing) {
      return NextResponse.json({ error: 'Chat room tidak ditemukan' }, { status: 404 })
    }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.chatMessage.deleteMany({ where: { roomId: id } })
    await db.chatRoom.delete({ where: { id } })

    return NextResponse.json({ message: 'Chat room berhasil dihapus' })
  } catch (error) {
